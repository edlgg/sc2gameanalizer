...
def find_similar_games(
    ...
    # Find all pro games with the same matchup (including mirror matchups)
    # e.g., if user plays PvT, also match TvP pro games
    cursor.execute("""
        ...
        AND (
            ...
            OR (player1_race = ? AND player2_race = ?)
        )
    ...
    for pro_game in pro_games:
        ...
        # Determine which player perspective to use for comparison
        # If it's a mirror matchup (PvT user vs TvP pro), swap perspective
        is_mirror = (user_p1_race == pro_p2_race and user_p2_race == pro_p1_race)
        pro_player_num = 2 if is_mirror else player_perspective
