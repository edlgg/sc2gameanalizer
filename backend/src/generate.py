...
def main():
    ...
    print("Initializing database (using DATABASE_URL)...")
    init_database()
    ...
    for i, replay_file in enumerate(replay_files, 1):
        ...
        try:
            ...
            print(f"SKIP: Corrupted/unparseable replay ({type(e).__name__}: {e})")
            ...
            print(f"SKIPPED {replay_file.name}: {type(e).__name__}: {e}")
            skipped.append((replay_file.name, str(e)))
            continue
